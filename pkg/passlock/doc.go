/*
Package passlock provides functions for encrypting data using a key derived from a user-provided pass phrase and payload salt.
This uses AES-256 encryption to encrypt the provided data, and [scrypt] to slowly generate a key from a pass phrase.

# Definition of terms:

  - plaintext: Data that has not been encrypted.
  - pass phrase: A user-defined phrase used in place of an encryption key. Longer is better.
...
A key and salt is generated from the given pass phrase. The salt is appended to the encrypted payload so the same key can be derived later given the same pass phrase.
Scrypt is memory and CPU hard, so it's impractical to brute force the salt to get the original pass phrase, provided that sufficient tuning values are provided to the KeyGenerator.
A normal user is really only expected to define an iteration count for key generation.

Pass the key, salt, and plaintext to the [Lock] function to encrypt the payload and append the salt to it.
Recover the key from the encrypted payload by passing the original pass phrase and the payload to [KeyGenerator.DeriveKey].
Pass the key and encrypted payload to the [Unlock] function to decrypt the payload and return the original plain text.
...
A [MultiLocker] is created using a KeyGenerator, and the encrypted payload is set by calling [MultiLocker.Lock] with the base pass phrase and plaintext.
...
A freshly read [MultiLocker] may not be changed in any way. Editing is enabled by calling [MultiLocker.EnableUpdate] with the base pass phrase.
After this call completes successfully, surrogate keys may be added or removed.
A new encrypted payload may only be set in a [MultiLocker] with the base pass phrase.
A WriteMultiLocker allows surrogate passphrases to be used to lock a new payload, instead of just the base pass phrase.

# General guidelines:
  - It's possible to customize the CPU cost, iteration count, and relative block size parameters directly for key generation. If you don't know what you're doing, then don't use SetIterations, SetCPUCost, or SetRelativeBlockSize.
  ...
  - If encrypted data is intended to be stored and available for a long time, choose the SetLongDelayIterations option for key generation.
  ...
  - The [MultiLocker] base key may be updated without invalidating all surrogate keys, because the base key's pass phrase is what is encrypted in surrogate key payloads. Reusing salt values is insecure.
